...
public:
	...
	bool Ielt::getMinmax(				// 10/06/99 AM.
		/*UP*/
		int &min,
		int &max
		);
	...
	void Ielt::internList(												// 12/12/98 AM.
		/*DU*/
		Dlist<Iarg>* &list,
		Htab* &htab
		);

	int Ielt::trigRating(												// 10/07/99 AM.
		/*UP*/
		bool &bestrating
		);
...
//		bool &spec,																// 04/29/01 AM.
	...
	bool Ielt::genEltrecurses(												// 11/04/99 AM.
		_TCHAR *sep,			// Element separator.
		_t_ostream &ofile
		);
	...
	static int Ielt::trigRatename(										// 10/07/99 AM.
		_TCHAR *name
		);

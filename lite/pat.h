...
public:
	...
	static bool Pat::matchSelect(
		Node<Pn> *select,
		Delt<Irule> *rules,
		Parse *parse
		);
	static bool Pat::matchSelect(		// 12/18/98 AM.
	...
	static bool Pat::traverseSelect(	// 11/30/98 AM.
	...
	static bool Pat::traverseSelect(	// 12/18/98 AM.
		Dlist<Iarg> *selects,	// Names of select nodes.
		Selt<Irule> *musts,		// Rules that must be tried.
		tHtab<Slist<Irule> > *htab,	// Hashed rule table.
		Parse *parse,				// Current analysis instance.
	...
	static bool Pat::multiSelect(		// 03/25/99 AM.
		Dlist<Iarg> *selects,	// Names of select nodes.
		Selt<Irule> *musts,				// Rules that must be tried.
		tHtab<Slist<Irule> > *htab,	// Hashed rule table.
		Parse *parse,				// Current analysis instance.
	...
	static bool Pat::multimatchSelect(
		Node<Pn> *select,			// Select, or chosen tree node.
		Selt<Irule> *musts,				// Rules that must be tried.
		tHtab<Slist<Irule> > *htab,	// Hashed rule table.
		Parse *parse,				// Current analysis instance.
	...
	static bool Pat::premultiSelect(		// 10/25/99 AM.
		Dlist<Iarg> *selects,	// Names of select nodes.
		Selt<Irule> *musts,				// Rules that must be tried.
		tHtab<Slist<Irule> > *htab,	// Hashed rule table.
		Parse *parse,				// Current analysis instance.
	...
	static bool Pat::premultimatchSelect(	// 10/25/99 AM.
		Node<Pn> *select,			// Select, or chosen tree node.
		Selt<Irule> *musts,				// Rules that must be tried.
		tHtab<Slist<Irule> > *htab,	// Hashed rule table.
		Parse *parse,				// Current analysis instance.
	...
	static bool Pat::pathSelect(		// 10/19/99 AM.
	...
	static bool Pat::pathmatchSelect(	// 10/19/99 AM.
		Node<Pn> *select,			// Select, or chosen tree node.
		Selt<Irule> *musts,				// Rules that must be tried.
		tHtab<Slist<Irule> > *htab,	// Hashed rule table.
		Parse *parse,				// Current analysis instance.
	...
	static bool Pat::matchNodes(		// 12/18/98 AM.
		Node<Pn> *start,			// Start node of range.
		Node<Pn> *end,				// End node of range, inclusive.
										// Null means move from start till end of list.
		Selt<Irule> *musts,				// Rules that must be tried.
	...
	static Selt<Irule> *Pat::resetRules(		// 12/18/98 AM.
	...
	static bool Pat::matchRules(		// 11/19/99 AM.
	...
	static bool Pat::matchRule(		// 11/19/99 AM.
		Nlppp *nlppp
		);
	...
	static bool Pat::matchFirst(		// 11/19/99 AM.
	...
	static bool Pat::matchLeft(		// 11/19/99 AM.
		Delt<Ielt> *trig,		// Trigger rule element.
		Nlppp *nlppp,			// 11/19/99 AM.
		/*DU*/
		Delt<Ielt>* &elt,		// Rule elt to continue matching against.
	...
	static bool Pat::matchRight(	// 11/19/99 AM.
	...
	static bool Pat::wildLeft(	// 11/19/99 AM.
		Delt<Ielt> *trig,		// Trigger rule element.
		Nlppp *nlppp,			// 11/19/99 AM.
		/*DU*/
		Delt<Ielt>* &elt,		// Rule elt to continue matching against.
	...
	static bool Pat::wildRight(	// 11/19/99 AM.
		Delt<Ielt> *trig,		// Trigger rule element.
		Nlppp *nlppp,			// 11/19/99 AM.
		/*DU*/
		Delt<Ielt>* &elt,		// Rule elt to continue matching against.
	...
	static bool Pat::matchRealRight(
		Ielt *ielt,			// Rule elt to continue matching against.
	...
	static bool Pat::trueMatchRight(
		Ielt *ielt,					// OPT // 10/06/99 AM.
		Node<Pn> *end,
		Nlppp *nlppp,        // 04/27/06 AM.
		/*DU*/
	...
	static bool Pat::Match(
		Ielt *ielt,				// OPT	// 10/05/99 AM.
		Node<Pn> *node
		);

	static bool Pat::modeMatch(
		Ielt *ielt,				// OPT	// 10/05/99 AM.
		Pn *pn,					// OPT	// 10/05/99 AM.
		/*UP*/
		bool &failmode				// 03/23/99 AM.
		);
	static bool Pat::modeMatch1(		// 12/04/98 AM.
	...
	static bool Pat::modeHash(			// 12/12/98 AM.
		_TCHAR *nname,				// Node name.
		Htab *htab,					// List's hash table.
		bool deaccent,                          // 03/27/05 AM.
		bool dejunk                               // 09/09/11 AM.
		);
	static bool Pat::modeMatches(					// 10/16/99 AM.
	...
	static bool Pat::modeFails(						// 10/16/99 AM.
		Dlist<Iarg> *list,					// EXCEPTS list.
		Htab *htab,										// 11/09/99 AM.
		_TCHAR *nname,							// Node name.
		enum Pntype typ,						// Node type.
	...
	static bool Pat::deaccentMatch(	// 01/28/05 AM.
		_TCHAR *ename,
		_TCHAR *nname
		);
	static bool Pat::dejunkMatch(	// 09/09/11 AM.
		_TCHAR *ename,
		_TCHAR *nname
		);
	static bool Pat::literalMatch(
		_TCHAR *ename,
		_TCHAR *nname
		);
	static bool Pat::singletMatch(
		Ielt *ielt,				// OPT	// 10/05/99 AM.
		Node<Pn> *node,
		/*UP*/
		bool &failmode						// 10/18/99 AM.
		);
	static bool Pat::treeMatch(					// 03/17/99 AM.
		Ielt *ielt,				// OPT	// 10/05/99 AM.
		Node<Pn> *node,
	...
	static bool Pat::preMatch(			// 11/30/98 AM.
//		Dlist<Ipre> *pres,
//		Pn *pn
//		);
...
//	static bool Pat::leafMatch(			// 01/31/99 AM.	// 06/16/05 AM.
//		Ielt *ielt,					// 10/05/99 AM.				// 06/16/05 AM.
//		Node<Pn> *node													// 06/16/05 AM.
//		);
	static bool Pat::singletExcept(	// 10/18/99 AM.
		Ielt *ielt,
		Node<Pn> *node,
		bool failmode
		);
	static bool Pat::treeExcept(		// 10/18/99 AM.
		Dlist<Iarg> *elist,
		Htab *htab,										// 11/09/99 AM.
		Node<Pn> *node,
		bool deaccent,									// 01/28/05 AM.
	...
	static bool Pat::preAction(			// 11/30/98 AM.
		_TCHAR *func,					// Name of pre action.
		Delt<Iarg> *args,			// Action's arguments.
//		Pn *pn						// Current node being matched.	// 06/16/05 AM.
		Node<Pn> *node				// 06/16/05 AM.
		);
	static bool Pat::matchEOF(			// 11/25/98 AM.
		Ielt *ielt,			// Current rule element.	// 10/05/99 AM.
		Node<Pn> *last				// Last node matched by rule.
		);
	static bool Pat::matchSTART(		// 01/30/99 AM.
		Ielt *ielt,			// Current rule element.	// 10/05/99 AM.
		Node<Pn> *last				// Last node matched by rule.
		);
	static bool Pat::matchEND(			// 01/21/99 AM.
		Ielt *ielt,			// Current rule element.	// 10/05/99 AM.
		Node<Pn> *last				// Last node matched by rule.
		);

	static bool Pat::eltMatches(		// 11/25/98 AM.
		Ielt *ielt,			// Current rule element.	// 10/05/99 AM.
		_TCHAR *name
		);

	static bool Pat::switchRight(		// 11/19/99 AM.
		Delt<Ielt> *trig,
		Nlppp *nlppp,				// 11/19/99 AM.
		/*DU*/
		Delt<Ielt>* &elt,
	...
	static bool Pat::execActions(			// 11/25/98 AM.
		Nlppp *nlppp							// 11/19/99 AM.
		);

	static bool Pat::checkActions(		// 11/27/98 AM.
		Nlppp *nlppp		// 11/19/99 AM.
		);
	static bool Pat::checkAction(			// 11/27/98 AM.
		Iaction *check,						// 01/13/00 AM.
		Nlppp *nlppp,							// 11/19/99 AM.
		/*UP*/
		bool &badname							// 01/13/00 AM.
		);
	static bool Pat::checkFinal(			// 11/27/98 AM.
		bool accept,				// If accepting rule match.
		Nlppp *nlppp				// 11/19/99 AM.
		);
	static bool Pat::checkEnd(          // 05/20/08 AM.
		Nlppp *nlppp
		);
	...
	static bool Pat::postAction(
		Iaction *post,					// 01/13/00 AM.
		Nlppp *nlppp,					// 11/19/99 AM.
		bool &badname					// 01/13/00 AM.
		);
	static bool Pat::postFinal(
		Nlppp *nlppp				// 11/19/99 AM.
		/*DU*/
//		Node<Pn>* &after // Saved node for continued matching.	// 12/14/03 AM.
	...
	static bool Pat::redSingler(											// 01/17/99 AM.
		Delt<Iarg> *args,		// Argument list for subphrase.
		Nlppp *nlppp															// 11/19/99 AM.
		);
	static bool Pat::redSinglex(											// 01/17/99 AM.
		Delt<Iarg> *args,		// Argument list for subphrase.
		Nlppp *nlppp															// 11/19/99 AM.
		);
	...
	static Node<Pn> *Pat::makeCollect(
	...
	static bool Pat::collectNth(											// 01/24/99 AM.
		Tree<Pn> *collect,
		long ord,
		/*UP*/
		Node<Pn>* &nstart,
	...
	static bool Pat::collectNthnew(										// 12/03/99 AM.
		Tree<Pn> *collect,
		long ord,
		/*UP*/
		Node<Pn>* &nstart,
	...
	static inline bool Pat::wildElt(Ielt *ielt);						// 10/05/99 AM.
	static inline bool Pat::optionalElt(Ielt *ielt);				// 10/05/99 AM.
	static inline bool Pat::warnWild(Irule *,Nlppp*);
	static inline Node<Pn> *Pat::nodeRight(
	...
	static inline Node<Pn> *Pat::nodeLeft(
		Node<Pn> *node,			// Current node.
	...
	static bool Pat::printr(												// 02/09/99 AM.
		long ord1,
		long ord2,
		Tree<Pn> *collect,
		Parse *parse,
	...
	static bool Pat::getr(													// 03/09/99 AM.
		long ord1,
		long ord2,
		Tree<Pn> *collect,
		Parse *parse,
	...
	static Node<Pn> *Pat::Layering(
		Node<Pn> *node,
		Dlist<Iarg> *attrs,
		Parse*																	// 08/22/02 AM.
		);
	static void Pat::Layering(												// 03/24/99 AM.
	...
	static bool Pat::execPhrase(	// 11/20/99 AM.
		Nlppp *nlppp				// 11/19/99 AM.
		);
	static bool Pat::layerElt(			// 03/24/99 AM.
	...
	static bool Pat::updateParent(									// 12/19/99 AM.
	...
	static Node<Pn> *Pat::baseNode(Node<Pn> *node);					// 01/24/99 AM.
	static _TCHAR *Pat::baseStr(Node<Pn> *node);						// 01/30/99 AM.
	static Node<Pn> *Pat::leafNode(Node<Pn> *node);					// 01/30/99 AM.
	static _TCHAR *Pat::rangeText(Node<Pn> *nstart, Node<Pn> *nend, Parse *parse);
					// 03/05/99 AM.
	...
	static Node<Pn> *Pat::group(											// 05/25/00 AM.
		Node<Pn> *node1,	// from
		Node<Pn> *node2,	// to
	...
	static bool Pat::traverseSelect(										// 05/13/00 AM.
#ifdef OLD_
		const _TCHAR *arr_select[],
		bool (rules)(int,bool&,Nlppp *),
#endif
	...
	static bool Pat::HtraverseSelect(									// 06/20/00 AM.
#ifdef OLD_
		const _TCHAR *arr_select[],
		const RHASH *must,
		const RHASH **htab,
	...
	static inline void Pat::resetRule(Node<Pn> *, Nlppp *);     // 05/18/00 AM.
//	static inline Node<Pn> *Pat::updateRestart(Nlppp*,Node<Pn> *);// 07/17/06 AM.
//	static inline void Pat::endRestart(Nlppp*);                 // 07/17/06 AM.
...
static inline Node<Pn> *Pat::updateRestart(	// RECOPT2
	Nlppp *nlppp,
...
static inline void Pat::endRestart(	// RECOPT2
	Nlppp *nlppp
	)
{
